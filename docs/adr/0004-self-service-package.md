...
3. **依赖化/层级化(参考 mattpocock/skills)**:总入口 `patent-router` 先路由材料来源、交付目标、专利类型和模板; `patent-application` 只负责访谈与编排;五个 model-invoked discipline 技能(claims/specification/drawings/compliance/filing)承载各自工件纪律。
4. **法律锚点单一来源**:`patent-standards` 只做分型文件索引；self-service 技能默认只保留文件指针，法律细节按需读取，不把 Rule basis 表格或条文解释复制进用户文稿。
...
- 包的可发现技能:7 个 self-service 技能(其中 router 1、application orchestrator 1、discipline 5)+共享 patent-standards+可选 tools/patents-search。
