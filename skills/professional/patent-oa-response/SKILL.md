...
description: "Draft an item-by-item response to a CN substantive-examination office action (审查意见答复 / 意见陈述书): parse the notice, classify each rejection, rebuild inventive-step argumentation on the three-step method, and choose the strategy (opinion only / amend claims / amend spec / correction) with estoppel checks. Use when the user asks to answer an office action, write an 意见陈述书, respond to a rejection on novelty / inventive step / clarity / support, or amend claims in response to an OA."
...
- Out of scope (sibling skills): 复审 → `patent-re-exam`; 无效 (请求 + 答辩) → `patent-invalidation`; 评价报告 → `patent-evaluation-report`; professional claim strategy (答复修改策略 / 分案 / 优先权 / 布局) → `patent-claim-strategy`; mechanical drafting → the self-service group.
