...
description: "Route a CN prosecution situation to the right discipline skill — OA 答复 / 复审 / 无效 / 评价报告 / 权利要求策略 — check each skill's input gate, and dispatch. Orchestrator only; the pleading logic lives in the discipline skills. Use when the user asks to respond to an office action, request re-examination, invalidate / defend a patent, request or use a patent evaluation report, or work out claim scope / amendment / division / priority strategy. Out of scope: 授权后业务 (FTO / 布局 / 维权 / 许可) and US prosecution."
...
- Out of scope: FTO / 专利布局 / 维权 / 许可 (授权后业务); US prosecution (US 组重做另起); 专利通俗解读 / 政策嗅探.
- Mechanical claim drafting for a fresh application: not this entry — route to the self-service `patent-intake` entry.
...
| Fresh application drafting | → self-service `patent-intake` | — |
| FTO / 布局 / 维权 / 许可 / US prosecution | out of scope — state so | — |
...
- The user's situation does not match any in-scope stage → state the boundary and what would be needed to help.
