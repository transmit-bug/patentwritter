...
description: 生成专利说明书附图(CN)。用 Graphviz 画结构图/流程图,按细则第21条管理图号与附图标记一致性,交叉核对说明书文字与附图,指定摘要附图;外观设计视图规则(六视图/视图清单)唯一执行版在 `../patent-application/references/design-points.md`,不在此重复(素材由发明人提供)。用户要求"画图""附图""流程图""示意图""摘要附图""六视图"时使用;也被 patent-application 入口技能调用。需要环境有 dot 命令(外观视图不依赖 dot)。
...
外观设计的"附图"是**图片或照片**(专利法第27条),不是 dot 渲染的线条示意图——**dot 不适用**。视图规则(按设计要点涉及的面数、六面正投影视图、省略视图声明、黑白/灰度、视图命名)唯一执行版在 `../patent-application/references/design-points.md`,本技能不重复规则。
