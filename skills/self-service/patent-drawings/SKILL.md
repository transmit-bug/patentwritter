...
description: 生成专利说明书附图(CN)。用 Graphviz 画结构图/流程图,按细则第21条管理图号与附图标记一致性,交叉核对说明书文字与附图,指定摘要附图。用户要求"画图""附图""流程图""示意图""摘要附图"时使用;也被 patent-application 入口技能调用。需要环境有 dot 命令。
...
```bash
dot -Tsvg fig1.dot -o fig1.svg   # svg 便于查看;递交用 jpg/png 黑白线条图(实务)
```
